...
| 🧩 **Module** | 📝 **Short Description**                                                                                                                                                                                                                                                         | 🔌 **Hardware / Deps**                                                                                                                                                                           | ⚖️ **License** | ⚠️ **Notes**                                                                                                       | 🚦 **Status** | 🔗 **Link** |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------- | ------------- | ----------- |
| **Pen3D**     | **2 buttons + optional scroll wheel**, **dual optical markers** for precise pose, **haptic motor**, **titanium tube** chassis. Optional **left-hand companion** with **mini-joystick** and **scroll wheel**. **Low-latency host notifications**; **no IR emitter** (camera-safe). | ESP32-S3 module, 2× tact switch, 1× rotary encoder, coin-vibe motor + driver, LiPo 150–300 mAh + charger (MCP73831/TP4056), power switch, passive markers (AprilTag/reflective), Ti tube, PCB.    | Apache-2.0     | BLE GATT (notify); deep-sleep wake-on-button; optional USB-CDC debug. Designed to coexist with 850 nm NIR tracking. | 🟡 Planned    | [Pen3D](https://github.com/xtanai/pen3d) |
| **HMDone**    | Prefer **marker-based tracking** on the headset (e.g., 3–4 passive markers) with **cameras external to the HMD** for higher precision; **disable/ignore built-in inside-out hand tracking** for CAD-grade work.                                                                   | Works with high-res HMDs (e.g., Pimax Crystal or Valve). **No extra hand controllers required** for MotionCoder.                                                                                           | Apache-2.0     | Use with multi-view/NIR rigs for best results; inside-out alone is not sufficient for precision CAD gestures.       | 🟠 Later      | [HMDone](https://github.com/xtanai/hmdone) |
| **HandNode**  | Universal **externally tracked hand tool** (shape-agnostic): grip / stylus / jog dial / handwheel / props. **Passive marker geometry** for stable 6DoF pose; designed for **high visibility** and low occlusion. Optional input via external modules. | **Passive markers** (AprilTag/reflective) + configurable mount geometry. Optional **BLE button/dial module** (ESP32-S3) if needed. | Apache-2.0     | Marker placement is the primary constraint (visibility first). Input meaning is defined by **profiles/config** (tool type, bindings).                                                                         | 🟠 Later      | [HandNode](https://github.com/xtanai/handnode) |
